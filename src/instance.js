...
export function alert(mes, title, cb){
  let promise;
  let PromiseIns;
...
export function confirm(mes, title = null, sucCb, errCb) {
  let promise;
  let PromiseIns;
  if ($.isFunction(title)) {
    errCb = sucCb;
...
export function prompt(mes,value, title = null, sucCb, errCb) {
  let promise;
  let PromiseIns;
  if ($.isFunction(title)) {
    errCb = sucCb;
...
let toastInstance;
export function toast(mes, time=1000){
  if(!toastInstance){
    let options={
      backdrop: false,
      content: mes,
